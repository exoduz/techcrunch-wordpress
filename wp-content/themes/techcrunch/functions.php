...
//get Techcrunch posts
function get_category_techcrunch_posts($query) {
	global $wp;
	...
	if ( !is_admin() && $query->is_main_query() ) { //available on non-admin page only and within main loop
	  if ( $wp->request == 'techcrunch' ) {
			...
			if( $query == false ) {
				//not cached, then do query
		    $args = array( 'cat' => 'techcrunch' );
		    $query = new WP_Query( $args );
			 
				wp_cache_set( 'techcrunch_cat_query', $query, '', 300 ); //cache results for 5 minutes
			}
...
add_action( 'pre_get_posts', 'get_category_techcrunch_posts');
